...
from src.processors import HansProcessor
...
class ModelArguments:
    ...
    random_segment: bool = field(default=False, metadata={"help": "used in the initialization "
                                                                  "of token type embedding?"})
...
class DynamicDataTrainingArguments(DataTrainingArguments):
    ...
    # For augmenting demonstrations with counterfactuals
    demo_augmentation: bool = field(
        default=False,
        metadata={"help": "Only use counterfactuals for the augmented examples"}
    )
    ...
    augment_train: bool = field(
        default=False,
        metadata={"help": "Augment train with counterfactuals"}
    )
    augmented_n: int = field(
        default=5,
        metadata={"help": "Number of counterfactuals to be augmented"}
    )
    augmented_bias_n: int = field(
        default=0,
        metadata={"help": "Number of examples supporting bias/heuristics to be augmented"}
    )
...
class DynamicTrainingArguments(TrainingArguments):
    ...
    use_teacher: bool = field(
        default=False,
        metadata={"help": "Use the original pre-trained model as teacher"}
    )

    optimizer: str = field(
        default="AdamW",
        metadata={"help": "Optimizer types: AdamW or "
                          "RecAdam https://www.aclweb.org/anthology/2020.emnlp-main.634.pdf"}
    )

    # RecAdam arguments
    recadam_anneal_fun: str = field(default="sigmoid",
                                    metadata={"help": "the type of annealing function in RecAdam. Default sigmoid"})
    recadam_anneal_k: float = field(default=0.01, metadata={"help": "k for the annealing function in RecAdam."})
    recadam_anneal_t0: int = field(default=250, metadata={"help": "t0 for the annealing function in RecAdam."})
    recadam_anneal_w: float = field(default=1.0,
                                    metadata={"help": "Weight for the annealing function in RecAdam. Default 1.0."})
    recadam_pretrain_cof: float = field(default=5000.0,
                                        metadata={"help": "Coefficient of the quadratic penalty in RecAdam. "
                                                          "Default 5000.0."})
...
def main():
    ...
    # prepare hans
    if data_args.task_name in ["mnli", "rte"]:
        hans_labels = HansProcessor.LABELS
        hans_subsets = HansProcessor.SUBSETS
        hans_datasets = []
        for hsubset in hans_subsets:
            for hlabel in hans_labels:
                # if hsubset == "constituent" and hlabel == "non-entailment":
                subset_data_args = dataclasses.replace(data_args, task_name="hans")
                hans_datasets.append(FewShotDataset(subset_data_args, tokenizer=tokenizer, mode="test",
                                                    use_demo=('demo' in model_args.few_shot_type),
                                                    hans_label=hlabel, hans_subset=hsubset))

    if data_args.task_name == "snli":
        scramble_data_args = dataclasses.replace(data_args, task_name="scramble-test")
        scramble_dataset = FewShotDataset(scramble_data_args, tokenizer=tokenizer, mode="test",
                                                 use_demo=('demo' in model_args.few_shot_type))
        scramble_long_data_args = dataclasses.replace(data_args, task_name="scramble-test-long")
        scramble_long_dataset = FewShotDataset(scramble_long_data_args, tokenizer=tokenizer, mode="test",
                                               use_demo=('demo' in model_args.few_shot_type))

    if data_args.task_name == "qqp":
        paws_data_args = dataclasses.replace(data_args, task_name="paws")
        paws_dataset = FewShotDataset(paws_data_args, tokenizer=tokenizer, mode="test",
                                      use_demo=('demo' in model_args.few_shot_type))
    ...
    teacher = None
    if training_args.use_teacher or training_args.optimizer == "RecAdam":
    ...
    def build_compute_metrics_fn(task_name: str) -> Callable[[EvalPrediction], Dict]:
        def compute_metrics_fn(p: EvalPrediction):
            ...
            if "hans" in task_name:
                # hack for 2-labels HANS
                temp_logits = np.array(logits)
                temp_logits[:, 0] = logits[:, [0, 2]].max(1)
                temp_logits[:, 2] = logits[:, [0, 2]].min(1)
                # temp_logits[:, 0] = logits[:, 0] + logits[:, 2]
                # temp_logits[:, 2] = 0.0
                logits = temp_logits

            if data_args.debug_mode:
                from scipy.special import softmax
                print("softmax\n", softmax(logits, axis=-1)[:10])
                print("logits\n",logits[:10])
            ...
            metrics = compute_metrics_mapping[task_name](task_name, preds, label_ids)
    ...
    if training_args.do_predict:
        ...
        if data_args.task_name == "mnli-syn":
            mnli_all_biased_data_args = dataclasses.replace(data_args, task_name="mnli-syn-all-biased")
            test_datasets.append(
                FewShotDataset(mnli_all_biased_data_args, tokenizer=tokenizer, mode="test", use_demo=('demo' in model_args.few_shot_type))
            )

        if data_args.task_name in ["mnli", "rte"]:
            # hans only loaded for mnli
            test_datasets = hans_datasets + test_datasets
            hans_avg = 0.0
            hans_summary = {}
        if data_args.task_name == "snli":
            test_datasets.append(scramble_dataset)
            test_datasets.append(scramble_long_dataset)
        if data_args.task_name == "qqp":
            test_datasets.append(paws_dataset)
        ...
        for test_dataset in test_datasets:
            ...
            if data_args.task_name in ["mnli", "rte"] and "hans" in test_dataset.test_task_name:
                hans_avg += test_result['eval_acc'] / len(hans_datasets)
                hans_summary[test_dataset.test_task_name] = test_result['eval_acc']

            output_test_file = os.path.join(
                training_args.output_dir, f"test_results_{test_dataset.test_task_name}.txt"
            )
            if trainer.is_world_master():
                with open(output_test_file, "w") as writer:
                    logger.info("***** Test results {} *****".format(test_dataset.test_task_name))
                    for key, value in test_result.items():
                        logger.info("  %s = %s", key, value)
                        writer.write("%s = %s\n" % (key, value))
                        final_result[test_dataset.test_task_name + '_test_' + key] = value

                if training_args.save_logit:
                    predictions = output.predictions
                    num_logits = predictions.shape[-1]
                    logits = predictions.reshape([test_dataset.num_sample, -1, num_logits]).mean(axis=0)
                    np.save(os.path.join(training_args.save_logit_dir, "{}-{}-{}.npy".format(test_dataset.test_task_name,
        ...
        if data_args.task_name in ["mnli"]:
            output_test_file = os.path.join(
                training_args.output_dir, "test_results_hans.txt"
            )
            if trainer.is_world_master():
                with open(output_test_file, "w") as writer:
                    logger.info("***** Test results hans all *****")
                    logger.info("%s = %s", "eval_acc", hans_avg)
                    logger.info(f"HANS summary:\n{str(hans_summary)}")
                    writer.write("%s = %s\n" % ("eval_acc", hans_avg))
                    writer.write(f"HANS summary:\n{str(hans_summary)}")
