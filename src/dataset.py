...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False, **kwargs):
        logger.info(f"******** Fewshot Dataset: {args.task_name}, {mode} ********")
        self.args = args
        ...
        cache_name = args.task_name
        cache_dir = cache_dir if cache_dir is not None else args.data_dir
        self.test_task_name = cache_name  # used in the test loop in run.py
        if args.task_name == "hans" and "hans_label" in kwargs:
            cache_name = f"{cache_name}_{kwargs['hans_label']}_{kwargs['hans_subset']}"
            self.test_task_name = cache_name
            cache_dir = os.path.join(args.data_dir, "../../../hans")
        ...
        with FileLock(lock_path):
            ...
            if os.path.exists(cached_features_file) and not args.overwrite_cache:
                ...
                self.support_examples = self.processor.get_support_examples(args.data_dir,
                                                                            augmented=args.augment_train,
                                                                            augmented_n=args.augmented_n,
                                                                            augmented_bias_n=args.augmented_bias_n)
                ...
                if mode == "dev":
                    ...
                    if args.task_name == "hans" and "hans_label" in kwargs:
                        self.query_examples = self.processor.get_test_examples(args.data_dir,
                                                                               filter_label=kwargs["hans_label"],
                                                                               filter_subset=kwargs["hans_subset"])
                    else:
                        self.query_examples = self.processor.get_test_examples(args.data_dir)
                else:
        ...
        if args.debug_mode and mode != "train":
            k = 1000
            if len(self.query_examples) > k:
                sampled_tuples = random.sample(list(enumerate(self.query_examples)), k=k)
                self.query_examples = [x[1] for x in sampled_tuples]
                self.sampled_query_indices = [x[0] for x in sampled_tuples]
                if self.use_demo and args.demo_filter:
                    self.query_emb = self.query_emb[self.sampled_query_indices]
            if len(self.support_examples) > k:
                sampled_tuples = random.sample(list(enumerate(self.support_examples)), k=k)
                self.support_examples = [x[1] for x in sampled_tuples]
                self.sampled_support_indices = [x[0] for x in sampled_tuples]
                if self.use_demo and args.demo_filter:
                    self.support_emb = self.support_emb[self.sampled_support_indices]

            logger.info(f"DEBUGGING MODE: sample only {k} instances")
        ...
        # keep track of augmented examples id
        augmented_indices = [ix for (ix, x) in enumerate(self.support_examples) if "augment" in x.guid]
                    ...
                    if self.num_labels == 1:
                        ...
                        for support_idx, score in sim_score:
                            if count_each_label[self.support_examples[support_idx].label] < limit_each_label:
                                ...
                                if args.debug_mode:
                                    print("    %.4f %s | %s" % (score, self.support_examples[support_idx].label, self.support_examples[support_idx].text_a)) # debug
                elif args.augment_train and self.use_demo and args.demo_augmentation and mode == "train":
                    # self.query_examples and self.support_indices supposed to be the same for train mode
                    if query_idx in augmented_indices:  # current query is an augmented example
                        context_indices = [support_idx for support_idx in augmented_indices
                                           if support_idx != query_idx]
                    else:  # augmented examples aren't used for demo for other queries -- try the other
                        context_indices = [support_idx for support_idx in support_indices
                                           if support_idx != query_idx]  # and support_idx not in augmented_indices]
                elif args.augment_train and self.use_demo and args.demo_augmentation and mode != "train":
                    # use only augmentation as demo
                    context_indices = [support_idx for support_idx in augmented_indices]
                else:
        ...
        if mode != "train":
            ...
            for query_idx, context_indices, bootstrap_idx in tqdm(self.example_idx):
